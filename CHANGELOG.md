...
- macOS 27: an arrangement that goes bad between placement retries is healed before the next retry, and a misplaced arrow no longer vetoes correct probes.
- macOS 27: a collapse is no longer blocked by the system's "Now Playing" or "Audio and Video Controls" item sitting between the separator and the arrow, nor by the arrow's ordering key having been pushed right up against the separator's (the app re-registers the arrow a few key units away to make room). Placement retries extended to five attempts over about a minute.
- macOS 27: when the arrow ends up left of the separator (a stale arrangement from an older build, or the system re-keying it), the app re-registers the arrow right of the separator by itself instead of asking for a drag.
- macOS 27: the bar no longer ends up expanded after a wake or display hot-plug. Screen changes are debounced until the display set has been stable for 1.5s, and a collapse whose fillers cannot be placed is retried after 2, 5 and 10 seconds before it rolls back to expanded.
- macOS 27: collapsing no longer fails when the active display differs from the one at launch or when the bar is crowded enough that the fillers push the separator into the overflow (frames are compared in one screen space, placeholder frames are ignored, and placement is validated against the arrow).
- macOS 27: diagnostic logging under subsystem `com.dwarvesv.minimalbar` (collapse/expand triggers, screen changes, sleep/wake, MenuBarAgent relaunches, filler placement, arrow healing, a periodic log-only audit of the collapsed state); see the runbook.
