...
| Nothing hides on macOS 27 and the separator sits to the right of the arrow | The app moves the arrow back to the right of the separator by itself at launch or on the next collapse. Then, with the bar **expanded**, ⌘-drag the `‹` arrow once (a nudge is enough): macOS 27 only pins an item's position after the user has dragged it, and an unpinned arrow can wander again when the bar is full. Never rearrange while collapsed: the blank fillers are on the bar then, a drag makes macOS re-derive every Hidden Bar item's position from geometry, and the arrow can end up behind a filler |
